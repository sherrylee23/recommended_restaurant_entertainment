...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _login() async {
    ...
    try {
      // 1. Silent Admin Check (Works for Desktop Admin)
      final adminData = await Supabase.instance.client
          .from('admin_profiles').select().eq('email', identifier).eq('password', password).maybeSingle();
      ...
      if (isBusiness) {
        ...
        if (bizData != null) {
          if (bizData['status'] == 'approved') {
            // If approved, send them to the new Business Main Page
            Navigator.pushReplacement(
                context,
                MaterialPageRoute(builder: (_) => BusinessMainNavigation(businessData: bizData))
            );
          } else {
            // If still pending, show the review page
