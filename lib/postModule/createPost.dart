  ...
  Future<void> _handlePostSubmission(LanguageProvider lp) async {
    ...
    try {
      ...
      for (var i = 0; i < _selectedMedia.length; i++) {
        ...
        final path =
            'posts/$currentProfileId/${DateTime.now().millisecondsSinceEpoch}_$i.jpg';
  ...
  Widget _buildMediaPicker(LanguageProvider lp) => GestureDetector(
    onTap: () async {
      ...
      if (i.isNotEmpty)
        setState(() {
          for (var x in i) {
            if (_selectedMedia.length < 9) _selectedMedia.add(File(x.path));
          }
        });
    },
    child: Container(
              ...
              onTap: () => Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => ImageGalleryViewer(
                    ...
                    initialIndex: index,
                    onDelete: (i) => setState(() => _selectedMedia.removeAt(i)),
                  ),
                ),
              ),
              child: Hero(
